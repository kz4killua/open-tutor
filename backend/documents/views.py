...
from .serializers import DocumentSerializer, FlashcardSerializer, MessageSerializer
...
from .utilities.vectorstore import (
    delete_vectors_from_vectorstore,
    upload_langchain_documents_to_vectorstore,
)
...
class DocumentList(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        langchain_documents = [
            LangChainDocument(
                page_text,
                metadata={"document_id": document.id, "page_number": page_number},
            )
...
class DocumentMessages(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        document = self.get_object()
        return document.messages.filter(role__in=("user", "assistant"))
    ...
    def post(self, request, *args, **kwargs):
        ...
        message_history = document.messages.filter(role__in=("user", "assistant"))

        response = StreamingHttpResponse(
            stream_message_response(
                user_message, system_message, assistant_message, message_history
            )
        )
...
class FlashcardFeedback(APIView):
    def post(self, request, *args, **kwargs):
        questions_correct = [
            flashcard.front
            for flashcard in Flashcard.objects.filter(
